...
git clone https://github.com/Valdiss-Valdiss/ai-marketing-ru-landing.git temp_skill
cd temp_skill
npm install -g
cd ..
rm -rf temp_skill
...
"plugin": [
  "...existing plugins...",
  "ПУТЬ_К_GLOBAL_NPM/node_modules/market-ru-landing/.opencode/plugins/market-ru-landing.js"
]
```
...
mkdir -p $SKILLS_DIR/market-ru-landing

# Скопировать SKILL.md
cp temp_skill/skills/market-ru-landing/SKILL.md $SKILLS_DIR/market-ru-landing/SKILL.md

# Скопировать агента (если есть)
if [ -f temp_skill/agents/market-ru-landing.md ]; then
    mkdir -p $AGENTS_DIR
    cp temp_skill/agents/market-ru-landing.md $AGENTS_DIR/market-ru-landing.md
fi

# Скопировать скрипты (если есть)
if [ -d temp_skill/scripts ]; then
    mkdir -p $SKILLS_DIR/market-ru-landing/scripts
    cp temp_skill/scripts/*.py $SKILLS_DIR/market-ru-landing/scripts/
fi
```
...
- Claude Code может использовать `/market-ru landing` команду
...
2. **Claude Code:** Файлы есть в ~/.claude/skills/market-ru-landing/
3. **Оба:** Команда `/market-ru landing` доступна
...
2. `npm uninstall -g market-ru-landing`
